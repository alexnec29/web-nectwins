...
$trainingLevels = $pdo
    ->query("SELECT id, name FROM training_level ORDER BY id")
    ->fetchAll(PDO::FETCH_ASSOC);

$trainingGoals = $pdo
    ->query("SELECT id, name FROM training_goal ORDER BY id")
    ->fetchAll(PDO::FETCH_ASSOC);
...
if (in_array($act, ['generate', 'save']) && isset($opt[$split][$part])) {
    try {
        ...
        $msg = str_contains($e->getMessage(), 'P2001')
...
if ($act === 'save' && $ex) {
    if (!$splitId || !$locId) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                CALL save_generated_workout(
                    :name, :duration,
                    :type_id, :level_id,
                    :split_id, :location_id,
